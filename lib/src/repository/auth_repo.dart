...
class AuthRepo extends GetxService {
  ...
  Future<Response> login(String email, String password) async {
    return await apiClient.postData('/account/login',
        body: {'email': email, 'password': password});
  }

  Future<Response> verifyCode(String code) async {
    return await apiClient
        .postData('/account/verify', body: {'verification': code});
  }

  Future<Response> registerUser(dynamic data) async {
    return await apiClient.postData('/account/register', body: data);
  }

  Future<Response> updateCourse(String courseId) async {
    return await apiClient
        .postData('/account/update-course', body: {'course': courseId});
  }
...
// save current /input token
  saveUserToken({String? token}) async {
    ...
    return await storage.write(key: 'userSession', value: _token);
    // return await sharedPrefences.setString('userToken', token);
  }

  remoUserToken() async {
    apiClient.token = null;
    apiClient.updateHeader(null);
    return await storage.delete(key: "userSession");
  }
}
